...
def _build_contract_report_pdf(
    ...
    # ── Header — same two-branch pattern as quote PDF ─────────────
    if logo_bytes:
        # Letterhead style: logo centered on white, gold line, then project info
        try:
            from PIL import Image as PILImage
            _pil = PILImage.open(io.BytesIO(logo_bytes))
            _lw, _lh = _pil.size
        ...
        story.append(Paragraph(
            ...
            sty("cr_sub", fontName="Helvetica-Bold", fontSize=11,
                textColor=DARK, alignment=TA_CENTER, spaceAfter=2)
        ))
        _dt2 = Table(
            [[Paragraph(report_date, sty("dt2", fontSize=10, textColor=MUTED)),
              Paragraph(f"Job: {project_name or '—'}",
                        sty("qn2", fontName="Helvetica-Bold", fontSize=10,
                            textColor=GOLD, alignment=TA_RIGHT))]],
            colWidths=[W * 0.5, W * 0.5]
        )
        _dt2.setStyle(TableStyle([
            ("TOPPADDING",    (0, 0), (-1, -1), 6),
            ("BOTTOMPADDING", (0, 0), (-1, -1), 4),
            ("LEFTPADDING",   (0, 0), (-1, -1), 0),
            ("RIGHTPADDING",  (0, 0), (-1, -1), 0),
        ...
        # Dark box: company name (gold) + tagline + report title
        hdr = Table([
            [Paragraph(company_name, sty("co", fontName="Helvetica-Bold", fontSize=20,
                       textColor=GOLD, alignment=TA_CENTER, leading=24))],
            [Paragraph(tagline, sty("tl", fontSize=9, textColor=SLATE, alignment=TA_CENTER))],
            [Paragraph("CONTRACT PROFITABILITY REPORT", sty("sub", fontName="Helvetica-Bold",
                       fontSize=12, textColor=LITE, alignment=TA_CENTER))],
            [Paragraph(report_date, sty("dt", fontSize=9, textColor=SLATE, alignment=TA_CENTER))],
        ], colWidths=[W])
        hdr.setStyle(TableStyle([
            ("BACKGROUND",    (0, 0), (-1, -1), DARK),
...
def show_login():
    ...
    .stApp {
        background: linear-gradient(135deg, #0a1628 0%, #0f2a5e 50%, #1a1f4e 100%);
        min-height: 100vh;
    }
    ...
    .stButton > button[kind="primary"] {
        background: linear-gradient(135deg, #ff4d4d, #f04747) !important;
    ...
    with col:
        ...
        st.markdown('<div style="display:flex; align-items:center; margin:16px 0;"><div style="flex:1; height:1px; background:rgba(255,255,255,0.1);"></div><div style="padding:0 12px; color:rgba(255,255,255,0.4); font-size:12px;">or</div><div style="flex:1; height:1px; background:rgba(255,255,255,0.1);"></div></div>', unsafe_allow_html=True)
...
/* ── Sidebar expand button: isotipo Lynsus ─────────────── */
...
/* Ocultar sidebar y su toggle completamente */
[data-testid="stSidebar"] {
    display: none !important;
}
[data-testid="collapsedControl"] {
...
/* Ocultar toolbar, badge, footer, deploy */
...
_spacer, _user_col = st.columns([8, 1])
with _user_col:
...
tab0, tab_est_config, tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
    "🏠 Dashboard",
    "📐 Estimación",
    "📊 Estimator",
    "📄 Client Quote",
...
with tab_est_config:
    ...
    # ── Logo branding al tope del sidebar ──
    _sb_logo = st.session_state.get("pdf_logo_bytes")
    if _sb_logo:
        _cols_logo = st.columns([1, 2, 1])
        with _cols_logo[1]:
            st.image(_sb_logo, use_container_width=True)
    else:
        st.markdown(
            ...
            f'style="width:56px;height:56px;object-fit:contain;border-radius:10px;" alt="logo"></div>',
            unsafe_allow_html=True
        )
...
with tab2:
    ...
    try:
        ...
        if _q_downloaded:
            ...
            _jname = (st.session_state.get("c_job_name") or job_location or "").strip()
            if _jname or _q_total_bid > 0:
                save_quote(
                    job_name=_jname or "—",
...
with tab5:
    ...
    with _ca_col_up1:
        st.markdown("""
<div style="text-align:center;padding:6px 0 2px 0;color:#64748b;font-size:13px;">
    📄 Drag and drop your GC Contract PDF here
</div>""", unsafe_allow_html=True)
        contract_pdf = st.file_uploader("Upload Contract PDF", type=["pdf"], key="contract_pdf_upload")
    with _ca_col_up2:
        st.markdown("""
<div style="text-align:center;padding:6px 0 2px 0;color:#64748b;font-size:13px;">
    📊 Drag and drop your G703 Excel file here
</div>""", unsafe_allow_html=True)
